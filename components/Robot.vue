...
<template>
  ...
  <div class="d-flex flex-column h-100" style="text-align: center;">
    <div>
      <div class="h5 mirte">MIRTE {{ robot }}</div>
      <p style="margin: auto; max-width: 500px; margin-bottom: 50px;" >{{ $t('robots.' + robot) }}</p>
      <NuxtLink v-if="robot !== 'master'" :to="localePath({ path: '/start/' + robot,  query: { level:  level } })" ><button class="btn my-button">{{ $t("robots.start") }} <span class="mirte">MIRTE</span> <b>{{robot}}</b>!</button></NuxtLink>
